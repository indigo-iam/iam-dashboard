...
export async function addScope(scope: RawScope): Promise<Notification> {
  ...
  if (response.ok) {
    ...
    return { type: "success", title: "Scope added" };
  }
  const msg = await response.text();
  return {
    type: "error",
    title: "Cannot add scope",
...
export async function deleteScope(scope: Scope): Promise<Notification> {
  ...
  if (response.ok) {
    revalidatePath("/scopes");
    return { type: "success", title: "Scope deleted" };
  }
  const msg = await response.text();
  return {
    type: "error",
...
export async function editScope(scope: Scope): Promise<Notification> {
  ...
  if (response.ok) {
    revalidatePath("/scopes");
    return { type: "success", title: "Scope saved" };
  }
  const msg = await response.text();
  return {
    type: "error",
    title: "Cannot save scope",
    description: `Error ${response.status} ${msg}`,
  };
}
